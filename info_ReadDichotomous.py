...
def graph_ts(df,stnname):
    '''
    # Function for plotting time series of 24 averages of PM data collected with a dichotomous sampler
     Parameters
    ----------
    df : TYPE panda dataframe containing PM2.5 and PM10 at three station in ug/m3
        
            DESCRIPTION.
    stnname : string with station name
    ...
    if stnname=='Independencia':
        par='EMF'
    elif stnname=='Parque'+' O'+'\'' +'Higgins':
        par='EMN'
    elif stnname=='Las Condes':
         par='EMM'
    
    
    titulo='PM 24 hrs averaged concentrations sampled with a dichotomous sampler at '+stnname
    nom1='PM10_'+par
    nom2='PM25_'+par
    ...
    plt.plot(PM10,':o',color='black',linewidth=0.5,markersize=5)             
    plt.plot(PM25,':x',color='red',linewidth=2,markersize=3)               
    plt.xlabel('Date', fontsize=18) ; 
    plt.ylabel('PM Concentration in $\mu g/m^3$', fontsize=18 )  
    plt.title(titulo, fontsize=20)                                       # Titulo busca el nombre con las ids CHR,RAP,SOMO.. 
    plt.legend(['PM$_{10}$','PM$_{2.5}$'],loc='upper center',ncol=3,frameon=False
               , fontsize=14 )                                                 # Legend
